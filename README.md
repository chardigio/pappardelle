...
- **Skip default PR/MR** — Opt-in flag (`skip_default_pr: true`, top-level or per-profile) that skips the placeholder PR/MR at workspace setup: no placeholder commit, no push. An existing PR/MR is still detected.
